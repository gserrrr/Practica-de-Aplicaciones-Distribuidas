...
public class DBActionsPeliculas {
    ...
    public String getCantidPorCadena(String par) {
        ...
        try {
            con.open();
            Statement st = con.getConection().createStatement();
            String sqlq = "select count(originaltitle) from peliculas where originaltitle like '% " + par + " %' OR originaltitle like '% " + par + "' OR originaltitle like '" +par+ " %' OR originaltitle like '" +par+"';";
            ResultSet rs = st.executeQuery(sqlq);
            int canti = 0;
            if (rs.next()) {
               canti = rs.getInt(1);
